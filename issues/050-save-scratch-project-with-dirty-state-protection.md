...
**Follow-up recorded (non-blocking, does not affect this PASS):** ~~Desktop application-close is not yet gated by the dirty check~~ **RESOLVED 2026-09-07** (commit adding `issue050_set_dirty` + `on_window_event` `CloseRequested` handler). The `beforeunload` handler now owns only the plain-browser fallback; in the packaged desktop app the frontend mirrors its dirty state into the shell via the ACL-gated `issue050_set_dirty` command (added to all four inventories: `build.rs` APP_COMMANDS, `permissions/main.toml`, `issue34.ts`, and the `generate_handler!` registration), and the shell's `on_window_event` handler (scoped to the `"main"` window, never the isolated issue-038 preview windows) intercepts `CloseRequested`, calls `api.prevent_close()` when dirty, and shows a native "Discard and quit / Cancel" confirmation dialog, force-destroying the window only on confirm. A companion `RunEvent::ExitRequested { code: None, .. }` handler covers whole-application quits that bypass per-window close (Windows/Linux quit, OS logout): it calls `api.prevent_exit()` when dirty, shows the same confirmation dialog, and on confirm clears the mirrored dirty flag and calls `AppHandle::exit(0)` (whose own `ExitRequested` carries `code = Some(0)` and so is not re-intercepted — no prompt loop). **macOS Cmd+Q / app-menu Quit** are a special case: the predefined Quit item calls AppKit `terminate:` directly and never fires `ExitRequested` (upstream bug tauri-apps/tauri#13778, dup of #12978). To gate those, the macOS menu is rebuilt (`issue050_build_macos_menu`) identically to `Menu::default` — App/File/Edit/View/Window/Help preserved, so Monaco keeps cut/copy/paste/undo — with the single exception that Quit is a custom `MenuItem` (id `issue050-quit`, accelerator `CmdOrCtrl+Q`) routed through `on_menu_event`, where the same dirty-check + confirmation dialog runs before `AppHandle::exit(0)`. Both discard paths clear the dirty mirror first so destroying the last window cannot cascade into a second prompt. **Known residual edge:** quitting via **Dock icon → Quit** still calls native `terminate:` and is not yet gated (Tauri has no `applicationShouldTerminate:` hook); closing all windows or Cmd+Q/app-menu Quit are covered. `cargo check`/`cargo test` (56 passing, incl. ACL drift-lock inventory checks) green. **Exit-gating verified (human, 2026-09-07):** all covered paths confirmed interactively — File→Close Window, Cmd+Q, and app-menu Quit each raise the discard prompt while dirty; Cancel keeps the app open and still dirty; Discard quits; quitting while clean exits immediately with no prompt; Edit menu cut/copy/paste/undo still work in the rebuilt macOS menu. Two minor suggestions also noted by the reviewer remain open: `with_extension("tmp"/"bak")` replaces rather than appends the extension (`Game1.tmp`/`Game1.bak`), and the toolbar Save button routes through `saveAs()` making `save()` currently dead UI until folder Save-All (issue 51) wires it.
