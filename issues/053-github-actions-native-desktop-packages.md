...
- One or more workflow files under `.github/workflows/` (e.g. `release.yml`) that:
  ...
  - Build and hash-verify the MonoGame WASM artifacts once (`scripts/build-monogame.sh` → `scripts/verify-monogame-artifacts.sh`, issues 4–5) and make them available to each platform leg (a shared upstream job that uploads the verified `artifacts/monogame` output, consumed by the three platform legs, is preferred over rebuilding the platform-independent WASM three times). Note that `build-monogame.sh` runs the MonoGame `build/Build.csproj` **"Build All"** target, which builds every framework (native SDL2/FAudio via Premake+CMake, Emscripten, DesktopGL, Android, iOS), all shaders (incl. Vulkan), tools and templates — so the job must provision the full MonoGame Linux build toolchain, not just the WASM bits: the SDL2/X11/audio `-dev` packages, Premake5, Java 17, the Vulkan SDK, the Android workload + dependencies, and Wine/fonts for MGFXC. Mirror MonoGame's own `install-wine@v1.0.3`, `install-fonts@v1.0.3`, and `install-android-dependencies@v1.0.3` composite actions and its `apt` package list exactly.
