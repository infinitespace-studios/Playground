...
4. [BLOCKED — premise corrected 2026-09-06] Retire the `issue038_*` window
   commands. **These are NOT "now-unused"** (the original wording was wrong). An
   audit found they remain load-bearing for currently-PASSING proofs:
   - **Issue 038 proof** (issue38.ts) — its `while(true){}` hostile game defeats
     cooperative stop by design; force-stop STRUCTURALLY requires
     `WebviewWindow::destroy()` (an in-page iframe with an infinite sync loop
     blocks the shared renderer thread and cannot be killed). Uses
     createIsolatedPreview / issue038_inject_script / _preview_window_exists /
     _monotonic_nanos. This is the whole reason issue 038 exists (PRD 8.3,
     feasibility Q18).
   - **Issue 039/040 proofs** — content mount still runs via createIsolatedPreview
     + issue038_destroy_all_previews.
   - **Issue 036 phase 2** — the bridge-command attack negatives (store_transfer,
     create_preview_window, bootstrap_preview, relay_to_preview,
     collect_bridge_messages) deliberately kept in the task-3 re-point.
   - **11 lifecycle/perf proofs** (23/25/27-30/37/041/048/049) still route through
     compileLoadStartIssue23 -> createIsolatedPreview.
   ARCHITECTURAL TENSION this exposes: issue 038's PASS certifies a hung preview
   CAN be force-stopped (isolated window). Issue 052 Option B DELIBERATELY gave
   up that guarantee for the LIVE path (freezing the editor on an in-tick
   infinite loop is "an acceptable learning experience"). So retiring the
   commands is not cleanup — it requires an explicit product decision on whether
   issue 038's isolated-window force-stop capability is retained at all, and
   migrating/retiring the 038/039/040 proofs first. BLOCKED pending that decision.
   (When unblocked: still ACL-locked across APP_COMMANDS, permissions/main.toml,
   generate_handler!, ISSUE034_APPROVED_COMMANDS, autogenerated tomls, and the
   build.rs/protocol.test.ts count assertions.)
