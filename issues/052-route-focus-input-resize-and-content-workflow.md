...
1. Route the live Run (issue 24 -> `compileLoadStartIssue23`) through the
...
2. Change force-stop semantics for the live path from
   `WebviewWindow::destroy()` to iframe removal/reload (the in-page path already
   supports this).
3. Re-point the security proofs (issues 33/34, and any of 35/36 that assert the
...
4. Retire the now-unused `issue038_*` window commands only as a SEPARATE,
   ...
   verification.
5. Likely BONUS: this should also eliminate the parked ~12s preview-startup
