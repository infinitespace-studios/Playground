...
} from "./scenario-load-controller";
import { withForcedPreviewRetirement } from "./scenario-run-controller";
import {
  createPreviewBridge,
  createPreviewIframe,
  loadPreviewIframe,
...
// Responsibility-split proof support modules (proof-only; never linked into the
// product graph). scenario-toolkit remains the core transfer-integrity /
// compile-and-load / issue22 / issue23 orchestrator and re-exports the extracted
// support surfaces below as a small barrel so the eight proof scenarios keep a
// single stable import site. Import edges flow one way (toolkit -> support), so
// there is no cycle.
import {
  preparePackagedProofRuntime,
} from "./packaged-proof-readiness";
import {
  createEmbeddedProofPreview,
} from "./embedded-preview-support";
...
export {
  ...
  compileToBuffers,
} from "./persistent-compile-support";
export {
  runInPagePreviewForProof,
  probeInPageNoWasmEvalBoot,
  createEmbeddedProofPreview,
  type InPageProofPreview,
...
// assigned these closures. Product never imports scenario-toolkit, so it never
// registers (and never links) this proof code.
registerContextSetup(() => {
...
async function compileAndLoadInternal(
  ...
  try {
    ...
    const proofOutcomes = verifyProofLoadOutcomes(probeLifecycle, () => ({
...
const normalLoadController = createProofLoadController({
  execute: async () => {
